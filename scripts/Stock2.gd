...
var stock_name = "Golden Doubloon Ventures"
var value = 125.0
var stock_count = 0
var trend_data = [5, 1]
...
func update_trend():
	...
	if trend_data.size() > 40:
		trend_data.pop_front()

func update_stock_prices():
	var change = randf_range(-0.7, 0.7) * value  # Medium random range for moderate volatility
	
	# Introduce occasional spikes or drops
	if randf() < 0.10:  # 10% chance for a high spike
		change *= randf_range(1.2, 2.0)
	elif randf() < 0.10:  # 10% chance for a sharp drop
		change *= randf_range(-1.2, -2.0)
