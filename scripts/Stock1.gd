...
func update_stock_prices():
	var change = randf_range(-1.0, 1.0) * value  # Larger random range for higher volatility
	
	# Introduce frequent and intense spikes or drops
	if randf() < 0.15:  # 15% chance for a high spike
		change *= randf_range(1.5, 3.0)
	elif randf() < 0.15:  # 15% chance for a sharp drop
		change *= randf_range(-1.5, -3.0)
